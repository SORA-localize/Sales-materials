...
const OrderSummary = ({ totalPrice, items }) => {
    ...
    return (
        <div className='w-full max-w-lg lg:max-w-[340px] bg-slate-50/30 border border-slate-200 text-slate-500 text-sm rounded-xl p-7'>
            <h2 className='text-xl font-medium text-slate-600'><T k="payment_summary" /></h2>
            <p className='text-slate-400 text-xs my-4'><T k="payment_method_label" /></p>
            ...
            <div className='flex gap-2 items-center mt-1'>
                ...
                <label htmlFor="STRIPE" className='cursor-pointer'><T k="stripe_payment" /></label>
            </div>
            <div className='my-4 py-4 border-y border-slate-200 text-slate-400'>
                <p><T k="address_label" /></p>
                {selectedAddress ? (
                    <div className='flex gap-2 items-center'>
                    ...
                    <div>
                        ...
                        <button className='flex items-center gap-1 text-slate-600 mt-1' onClick={() => setShowAddressModal(true)}>
                            <T k="add_address" /> <PlusIcon size={18} />
                        </button>
                    </div>
                )}
            </div>
            <div className='pb-4 border-b border-slate-200'>
                <div className='flex justify-between'>
                    <div className='flex flex-col gap-1 text-slate-400'>
                        <p><T k="subtotal_label" /></p>
                        <p><T k="shipping_label" /></p>
                        {coupon && <p><T k="coupon_label" /></p>}
                    </div>
